"""
Email Model Evaluator for Phishing Detection
...
class EmailModelEvaluator:
    """Email model evaluation and testing framework"""
    
    def __init__(self):
        self.email_analyzer = None
        self.email_data_simulator = EmailDataSimulator()
    ...
    async def initialize(self):
        ...
        logger.info("Email Model Evaluator initialized")
    
    async def evaluate_model(self, test_data: List[Dict[str, Any]], 
                           model_name: str = "Email Analyzer") -> ModelEvaluation:
        """
        Evaluate model performance on test data
        Args:
            test_data: List of test emails with labels
        ...
        for i, item in enumerate(test_data):
            try:
                # Get prediction
                result = await self.email_analyzer.analyze_email(
                    item["subject"], 
                    item["body"], 
                    item.get("sender", "test@example.com")
                )
                
                # Create test result
                test_result = TestResult(
                    message=f"Subject: {item['subject']}\nBody: {item['body'][:100]}...",
                    expected_label=item["is_phishing"],
                    predicted_label=result["is_phishing"],
                    confidence=result["confidence"],
                    is_correct=result["is_phishing"] == item["is_phishing"],
        ...
        evaluation = ModelEvaluation(
            ...
            dataset_name="Email Test Dataset",
        ...
        for batch_size in batch_sizes:
            ...
            for batch in batches:
                ...
                correct = sum(1 for i, result in enumerate(batch_results) 
                            if result["is_phishing"] == batch[i]["is_phishing"])
            ...
            avg_time_per_email = avg_time_per_batch / batch_size if batch_size > 0 else 0
...
"""
        ...
        for i, result in enumerate(correct_predictions[:5]):
            report += f"{i+1}. **Email**: {result.message[:100]}...\n"
            report += f"   **Expected**: {'Phishing' if result.expected_label else 'Legitimate'}\n"
            report += f"   **Predicted**: {'Phishing' if result.predicted_label else 'Legitimate'}\n"
    ...
    async def evaluate_phishing_types(self, test_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Evaluate performance by phishing type
        Args:
            test_data: Test dataset with phishing types
        Returns:
            Performance by phishing type
        """
        logger.info("Evaluating performance by phishing type")
        
        # Group by phishing type
        phishing_types = {}
        for item in test_data:
            if item["is_phishing"] and "phishing_type" in item:
                phishing_type = item["phishing_type"]
                if phishing_type not in phishing_types:
                    phishing_types[phishing_type] = []
                phishing_types[phishing_type].append(item)
        
        type_results = {}
        for phishing_type, items in phishing_types.items():
            logger.info(f"Evaluating {phishing_type}: {len(items)} samples")
            
            # Evaluate this type
            evaluation = await self.evaluate_model(items, f"Email Analyzer - {phishing_type}")
            
            type_results[phishing_type] = {
