...
export default function VideoChannelsPage() {
  ...
  const handleAnalyzeVideo = async () => {
    ...
    try {
      const result = await analyzeVideoGemini(selectedFile)
  ...
  return (
    <MainLayout>
      ...
      <header className="flex items-center justify-between p-6 border-b border-[#21262d]">
        <div>
          <h2 className="text-3xl font-bold">Video Channels</h2>
             ...
             <div className="bg-[#0D1117] rounded-lg border border-[#21262d] p-8 text-center">
               ...
               <div className="flex justify-center mb-6">
                 <div className="w-24 h-24 bg-[var(--accent-teal)]/20 rounded-full flex items-center justify-center border-2 border-[var(--accent-teal)]/30">
                   <Video className="w-12 h-12 text-[var(--accent-teal)]" />
                 </div>
               </div>

               {/* Upload Title */}
               <h3 className="text-2xl font-bold text-white mb-2">Upload Video for Analysis</h3>
               <p className="text-gray-400 mb-8">Upload a video file or record directly to analyze for deepfake detection</p>
                 ...
                 <div className="mt-6 p-4 bg-[#161b22] rounded-lg border border-[#21262d]">
                   <div className="flex items-center justify-between mb-3">
                     <div className="flex items-center gap-3">
                       <FileVideo className="w-5 h-5 text-[var(--accent-teal)]" />
              ...
              {(() => {
                ...
                return (
                  <>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="text-center">
                          <div className="text-lg font-bold text-white">
                            {(result.gemini_analysis.face_quality_score * 100).toFixed(0)}%
                          </div>
                          <p className="text-xs text-gray-400">Face Quality</p>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-bold text-white">
                            {(result.gemini_analysis.lip_sync_score * 100).toFixed(0)}%
                          </div>
                          <p className="text-xs text-gray-400">Lip Sync</p>
                        </div>
                      </div>
                    )}
                    ...
                    {result.gemini_analysis?.artificial_indicators && result.gemini_analysis.artificial_indicators.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-white font-semibold mb-2">Technical Indicators</h4>
                         <div className="flex flex-wrap gap-2">
                           {result.gemini_analysis.artificial_indicators.map((indicator: string, index: number) => (
                             <span
                               key={index}
                               className="bg-orange-500/20 text-orange-400 text-xs font-medium px-2 py-1 rounded-full"
                             >
                               {indicator}
                             </span>
                           ))}
